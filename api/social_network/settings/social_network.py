...
SOCIAL_NETWORK_FEATURES = env.list(
    "SOCIAL_NETWORK_FEATURES",
    default=[],
)

# Reset password link lifetime interval (in seconds). By default: 1 hour.
SOCIAL_NETWORK_RESET_PASSWORD_EXPIRATION_DELTA = timedelta(
    seconds=env.int(
        "SOCIAL_NETWORK_RESET_PASSWORD_EXPIRATION_DELTA",
        default=3600,
    ),
)

if "LOG_SQL" in SOCIAL_NETWORK_FEATURES:  # pragma: no cover
    LOGGING = {
        "version": 1,
        ...
        "formatters": {
            ...
            "sql": {
                "()": "social_network.loggers.SQLFormatter",
