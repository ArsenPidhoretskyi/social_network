...
PASSWORD_SERVICE_PATH = "social_network.apps.accounts.services.password.PasswordService"  # nosec
...
def test_reset_password_signature_failure(mocker, settings, exception):
    ...
    settings.social_network_RESET_PASSWORD_EXPIRATION_DELTA = 42
    mocked_change_password = mocker.patch(f"{PASSWORD_SERVICE_PATH}.change_password")
...
def test_reset_password_user_failure(mocker):
    ...
    reset_password_signature = TimestampSigner().sign(str(uuid.uuid4()))
    mocked_change_password = mocker.patch(f"{PASSWORD_SERVICE_PATH}.change_password")
